...
def filter_playlist(
    ...
    *,
    ...
    for entry in copy.deepcopy(playlist):
        ...
        entry["settings"]["map"] = get_filtered_map_name(entry["settings"]["map"])
        ...
        try:
            ...
            if entry["type"] in (
                ...
                entry["type"] = "bascenev1lib.game.kingofthehill.KingOfTheHillGame"
            if entry["type"] in (
                ...
                entry["type"] = "bascenev1lib.game.capturetheflag.CaptureTheFlagGame"
            if entry["type"] in (
            ...
            logging.warning("Import failed while scanning playlist '%s': %s", name, exc)
        except Exception:
