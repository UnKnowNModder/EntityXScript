...
class ServerController:
    ...
    def print_client_list(self) -> None:
        ...
        col1 = 10
        col2 = 16
        out = f"{Clr.BLD}" f"{title1:<{col1}} {title2:<{col2}} {title3}" f"{Clr.RST}"
    ...
    def _execute_shutdown(self) -> None:
        ...
        if self._shutdown_reason is ShutdownReason.RESTARTING:
            ...
            print(f"{Clr.SBLU}Exiting for server-restart" f" at {timestrval}.{Clr.RST}")
        else:
            bascenev1.broadcastmessage(
            ...
            print(
                f"{Clr.SBLU}Exiting for server-shutdown" f" at {timestrval}.{Clr.RST}"
            )
        with babase.ContextRef.empty():
    ...
    def _get_session_type(self) -> type[bascenev1.Session]:
        ...
        raise RuntimeError(f'Invalid session_type: "{self._config.session_type}"')
    ...
    def _launch_server_session(self) -> None:
        ...
        if sessiontype is bascenev1.FreeForAllSession:
            ...
            appcfg["Free-for-All Playlist Randomize"] = self._config.playlist_shuffle
        elif sessiontype is bascenev1.DualTeamSession:
            appcfg["Team Tournament Playlist Selection"] = self._playlist_name
            appcfg["Team Tournament Playlist Randomize"] = self._config.playlist_shuffle
        elif sessiontype is bascenev1.CoopSession:
        ...
        bascenev1.set_public_party_public_address_ipv4(self._config.public_ipv4_address)
        bascenev1.set_public_party_public_address_ipv6(self._config.public_ipv6_address)

        bascenev1.set_public_party_enabled(self._config.party_is_public)

        bascenev1.set_player_rejoin_cooldown(self._config.player_rejoin_cooldown)

        bascenev1.set_max_players_override(self._config.session_max_players_override)
