...
class StoreSubsystem:
    ...
    def get_store_item_name_translated(self, item_name: str) -> babase.Lstr:
        ...
        if item_name.startswith("characters."):
            return babase.Lstr(translate=("characterNames", item_info["character"]))
        ...
        if item_name == "upgrades.infinite_runaround":
            return babase.Lstr(translate=("coopLevelNames", "Infinite Runaround"))
        if item_name == "upgrades.infinite_onslaught":
            return babase.Lstr(translate=("coopLevelNames", "Infinite Onslaught"))
        raise ValueError("unrecognized item: " + item_name)

    def get_store_item_display_size(self, item_name: str) -> tuple[float, float]:
        """(internal)"""
        if item_name.startswith("characters."):
    ...
    def get_store_items(self) -> dict[str, dict]:
        ...
        if babase.app.classic.store_items is None:
            ...
            babase.app.classic.store_items = {
                ...
                "icons.flag_us": {
                    "icon": babase.charstr(babase.SpecialChar.FLAG_UNITED_STATES)
                },
                "icons.flag_mexico": {
                ...
                "icons.flag_uk": {
                    "icon": babase.charstr(babase.SpecialChar.FLAG_UNITED_KINGDOM)
                },
                "icons.flag_canada": {
                ...
                "icons.flag_uae": {
                    "icon": babase.charstr(babase.SpecialChar.FLAG_UNITED_ARAB_EMIRATES)
                },
                "icons.flag_qatar": {
                ...
                "icons.flag_czech_republic": {
                    "icon": babase.charstr(babase.SpecialChar.FLAG_CZECH_REPUBLIC)
                },
                ...
                "icons.fedora": {"icon": babase.charstr(babase.SpecialChar.FEDORA)},
                "icons.hal": {"icon": babase.charstr(babase.SpecialChar.HAL)},
                "icons.crown": {"icon": babase.charstr(babase.SpecialChar.CROWN)},
                "icons.yinyang": {"icon": babase.charstr(babase.SpecialChar.YIN_YANG)},
                "icons.eyeball": {"icon": babase.charstr(babase.SpecialChar.EYE_BALL)},
                "icons.skull": {"icon": babase.charstr(babase.SpecialChar.SKULL)},
                "icons.heart": {"icon": babase.charstr(babase.SpecialChar.HEART)},
                "icons.dragon": {"icon": babase.charstr(babase.SpecialChar.DRAGON)},
                "icons.helmet": {"icon": babase.charstr(babase.SpecialChar.HELMET)},
                "icons.mushroom": {"icon": babase.charstr(babase.SpecialChar.MUSHROOM)},
                "icons.ninja_star": {
                ...
                "icons.spider": {"icon": babase.charstr(babase.SpecialChar.SPIDER)},
                "icons.fireball": {"icon": babase.charstr(babase.SpecialChar.FIREBALL)},
                "icons.mikirog": {"icon": babase.charstr(babase.SpecialChar.MIKIROG)},
                "icons.explodinary": {
    ...
    def _calc_count_for_tab(
        ...
        for section in tabval:
            for item in section["items"]:
                ticket_cost = plus.get_v1_account_misc_read_val("price." + item, None)
        ...
        try:
            ...
            if tab == "extras":
                ...
                if app.classic.pro_sale_start_time is None:
                    ...
                    if "PSTR" in config:
                        app.classic.pro_sale_start_time = int(babase.apptime() * 1000)
                        ...
                        if start_duration is not None:
                            ...
                            app.classic.pro_sale_start_val = 60000 * start_duration
                ...
                val: int | None = max(
                    ...
                    - (
                        int(babase.apptime() * 1000.0) - app.classic.pro_sale_start_time
                    ),
                )
