...
class AccountV1Subsystem:
    ...
    def cache_tournament_info(self, info: Any) -> None:
        ...
        for entry in info:
            cache_entry = self.tournament_info[entry["tournamentID"]] = copy.deepcopy(
                entry
            )
    ...
    def get_purchased_icons(self) -> list[str]:
        ...
        for item_name, item in list(store_items.items()):
            if item_name.startswith("icons.") and item_name in classic.purchases:
    ...
    def ensure_have_account_player_profile(self) -> None:
        ...
        if not babase.can_display_chars(plus.get_v1_account_display_string(full=False)):
            return
    ...
    def on_account_state_changed(self) -> None:
        ...
        if plus.get_v1_account_state() == "signed_in" and self.pending_promo_codes:
    ...
    def add_pending_promo_code(self, code: str) -> None:
        ...
        if plus is None:
            ...
            logging.warning("Error adding pending promo code; plus not present.")
            babase.screenmessage(babase.Lstr(resource="errorText"), color=(1, 0, 0))
