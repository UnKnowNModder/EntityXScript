...
class OSMusicPlayer(MusicPlayer):
    ...
    def _on_play_folder_cb(
        ...
        if error is not None:
            rstr = babase.Lstr(resource="internal.errorPlayingMusicText").evaluate()
            if isinstance(result, str):
                ...
                err_str = rstr.replace("${MUSIC}", "<multiple>") + "; " + str(error)
    ...
    def run(self) -> None:
        ...
        try:
            ...
            if not all_files:
                do_log_error = False
                raise RuntimeError(
                    babase.Lstr(resource="internal.noMusicFilesInFolderText").evaluate()
                )
            babase.pushcall(
