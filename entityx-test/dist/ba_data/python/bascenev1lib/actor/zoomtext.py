...
class ZoomText(bs.Actor):
        ...
        *,
        ...
        if shiftposition is not None:
            ...
            if jitter > 0.0:
                bs.timer(
                    shiftdelay + 0.25,
                    bs.WeakCall(self._jitter, positionadjusted2, jitter * scale),
                )
    ...
    def _jitter(self, position: tuple[float, float], jitter_amount: float) -> None:
        if not self.node:
            return
        ...
        for index, attr in enumerate(["input0", "input1"]):
            ...
            for _i in range(10):
                keys[timeval] = (
                    position[index] + (random.random() - 0.5) * jitter_amount * 1.6
                )
