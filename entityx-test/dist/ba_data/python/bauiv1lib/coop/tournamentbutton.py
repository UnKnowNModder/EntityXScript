...
class TournamentButton:
    ...
    def _show_leader(self) -> None:
        ...
        if tournament_id is None or self.leader is None or len(self.leader[2]) != 1:
    ...
    def _update_lock_state(self) -> None:
        ...
        enabled = self.required_league is None and bui.app.classic.is_game_unlocked(
            self.game
        )
        bui.buttonwidget(
            edit=self.button,
    ...
    def update_for_data(self, entry: dict[str, Any]) -> None:
        ...
        prize_y_offs = (
            34 if "prizeRange3" in entry else 20 if "prizeRange2" in entry else 12
        )
        ...
        assert bui.app.classic is not None
        self.game = bui.app.classic.accounts.tournament_info[self.tournament_id]["game"]
        ...
        bui.textwidget(edit=self.time_remaining_out_of_text, text=out_of_time_text)
        ...
        #     )
        # else:
        max_players = bui.app.classic.accounts.tournament_info[self.tournament_id][
            "maxPlayers"
        ]
        ...
        self.allow_ads = allow_ads = entry["allowAds"] if USE_ENTRY_FEES else False

        final_fee = (
            None if fee_var is None else plus.get_v1_account_misc_read_val(fee_var, "?")
        )
        ...
        if fee_var is None:
            ...
            if final_fee == 0:
                ...
                final_fee_str = bui.charstr(bui.SpecialChar.TICKET_BACKING) + str(
                    final_fee
                )
