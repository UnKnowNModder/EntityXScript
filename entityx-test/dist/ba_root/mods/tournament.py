...
old_on_begin = bascenev1lib.activity.multiteamvictory.TeamSeriesVictoryScoreScreenActivity.on_begin

def new_on_begin(self) -> None:
	if match := bs.storage.tournament.match:
		# a series has ended-.
		series.append(1) # whatever value is fine, we js want the length to increase by 1.
...
@on_command(name="/discard", aliases=["/endmatch", "/rmmatch"], authority = Authority.LEADER)
