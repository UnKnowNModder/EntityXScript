...
class MessageProtocol:
    ...
    def __init__(
        ...
        *,
        ...
        self.response_types_by_id: dict[int, type[Response] | type[SysResponse]] = {}
        self.response_ids_by_type: dict[type[Response] | type[SysResponse], int] = {}
        for m_id, m_type in message_types.items():
            ...
            assert m_id >= 0
            assert is_ioprepped_dataclass(m_type) and issubclass(m_type, Message)
        ...
        for r_id, r_type in response_types.items():
            ...
            assert r_id >= 0
            assert is_ioprepped_dataclass(r_type) and issubclass(r_type, Response)
        ...
        if __debug__:
            ...
            for m_id, m_type in message_types.items():
                ...
                assert m_rtypes, f"Message type {m_type} specifies no return types."
        ...
        self.remote_errors_include_stack_traces = remote_errors_include_stack_traces
    ...
    def error_to_response(self, exc: Exception) -> tuple[SysResponse, bool]:
        ...
        if self.forward_communication_errors and isinstance(exc, CommunicationError):
            return (
                ErrorSysResponse(
                    error_message=str(exc),
    ...
    def _to_dict(self, message: Any, ids_by_type: dict[type, int], opname: str) -> dict:
        """Encode a message to a json string for transport."""

        m_id: int | None = ids_by_type.get(type(message))
        if m_id is None:
            raise TypeError(
                f"{opname} type is not registered in protocol:" f" {type(message)}"
            )
    ...
    def _from_dict(
        ...
        if msgtype is None:
            raise UnregisteredMessageIDError(f"Got unregistered {opname} id of {m_id}.")
    ...
    def _get_module_header(
        ...
        for msgtype in msgtypes:
            tpimports.setdefault(msgtype.__module__, []).append(msgtype.__name__)
        ...
        for rsp_tp in rsptypes:
            ...
            if rsp_tp is EmptySysResponse or rsp_tp is ErrorSysResponse:
                continue
            if single_message_type and part == "sender" and rsp_tp is not Response:
                ...
                tpimports.setdefault(rsp_tp.__module__, []).append(rsp_tp.__name__)
