        ...
        *,
        ...
        try:
            ...
            if name == "__default__":
                ...
                try:
                    plst = bui.app.config[self._pvars.config_name + " Playlists"][name]
                except Exception:
                    print(
        ...
        playlist_name: str | bui.Lstr = (
            self._pvars.default_list_name if playlist == "__default__" else playlist
        )
            ...
            for col in range(columns):
                ...
                if tex_index < len(map_textures):
                    ...
                    owned = not (
                        ...
                        or ("is_unowned_game" in entry and entry["is_unowned_game"])
                    )
                        ...
                        if not owned:
                            desc = bui.Lstr(
                                ...
                                subs=[
                                    ...
                                    (
                                        "${UNLOCK}",
                                        bui.Lstr(resource="unlockThisInTheStoreText"),
                                    ),
                                ],
                            )
                    ...
                    btn = bui.buttonwidget(
                        ...
                        on_activate_call=bui.Call(bui.screenmessage, desc, desc_color),
                        label="",
                        color=(1, 1, 1),
                        autoselect=True,
        ...
        self._series_length_numedit = ConfigNumberEdit(
            ...
            configkey=("FFA" if self._sessiontype is bs.FreeForAllSession else "Teams")
            + " Series Length",
            ...
            fallback_value=(24 if self._sessiontype is bs.FreeForAllSession else 7),
            f=0,
        )
        ...
        if self._custom_colors_names_button is not None:
            for btn in bottom_row_buttons:
                bui.widget(edit=btn, down_widget=self._custom_colors_names_button)
            if show_shuffle_check_box:
                bui.widget(
        ...
        bui.widget(edit=self._ok_button, up_widget=self._show_tutorial_check_box)
