...
class GamepadSettingsWindow(bui.MainWindow):
    ...
    def set_start_button_activates_default_widget_value(self, value: bool) -> None:
        """(internal)"""
        assert self._settings is not None
        if value:
    ...
    def _save(self) -> None:
        ...
        if self._inputdevice:
            dst = classic.get_input_device_config(self._inputdevice, default=True)
...
class AwaitGamepadInputWindow(bui.Window):
    ...
    def _event_callback(self, event: dict[str, Any]) -> None:
        ...
        if self._input and input_device and input_device.name == self._input.name:
