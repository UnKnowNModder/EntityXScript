...
class ControlsGuide(bs.Actor):
    ...
    @staticmethod
    def _meaningful_button_name(device: bs.InputDevice, button_name: str) -> str:
        """Return a flattened string button name; empty for non-meaningful."""
        if not device.has_meaningful_button_names:
            return ""
        assert bs.app.classic is not None
        button = bs.app.classic.get_input_device_mapped_value(device, button_name)
    ...
    def _check_fade_in(self) -> None:
        ...
        if touchscreen is not None:
            ...
            input_devices = [i for i in input_devices if i and i is not touchscreen]
            fade_in = False
            if input_devices:
    ...
    def _fade_in(self) -> None:
        ...
        if self._lifespan is not None:
            bs.timer(self._lifespan, bs.WeakCall(self.handlemessage, bs.DieMessage()))
        self._update()
        self._update_timer = bs.Timer(1.0, bs.WeakCall(self._update), repeat=True)
    ...
    def _update(self) -> None:
        ...
        for device in input_devices:
            ...
            if all_keyboards:
                ...
                up_button_names.add(self._meaningful_button_name(device, "buttonUp"))
        ...
        run_text = bs.Lstr(
            ...
            subs=[
                ...
                (
                    ...
                    bs.Lstr(
                        resource=(
                            "holdAnyKeyText" if all_keyboards else "holdAnyButtonText"
                        )
                    ),
                ),
            ],
