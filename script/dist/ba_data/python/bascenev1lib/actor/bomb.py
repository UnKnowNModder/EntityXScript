...
class Bomb(bs.Actor):
    ...
    def __init__(
        ...
        *,
        ...
        if self.bomb_type not in ("land_mine", "tnt"):
            ...
            bs.timer(fuse_time, bs.WeakCall(self.handlemessage, ExplodeMessage()))
    ...
    def get_source_player[PlayerT: bs.Player](
        ...
        return player if isinstance(player, playertype) and player.exists() else None
    ...
    def _handle_splat(self) -> None:
        node = bs.getcollision().opposingnode
        if node is not self.owner and bs.time() - self._last_sticky_sound_time > 1.0:
    ...
    def arm(self) -> None:
        ...
        if self.bomb_type == "land_mine":
            ...
            bs.timer(
                0.25,
                bs.WeakCall(self._add_material, factory.land_mine_blast_material),
            )
        elif self.bomb_type == "impact":
            intex = (
...
class TNTSpawner:
    ...
    def __init__(self, position: Sequence[float], respawn_time: float = 20.0):
        ...
        self._update_timer = bs.Timer(1.1, bs.WeakCall(self._update), repeat=True)
